...
import { ProductVariant, ProductVariantSize } from "@/types/product";
...
export function useProductVariants(productVariants: ProductVariant[]) {
  ...
  const productSizes = productVariants.reduce((previousValue, currentValue) => {
    if (!previousValue.find((size) => isSameSize(size, currentValue.size))) {
  ...
  function selectAroma(aroma: string) {
    const aromaWithSameSize = productVariantsByAroma[aroma].find((variant) =>
