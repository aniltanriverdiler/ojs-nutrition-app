...
const MenuNavigation = () => {
  ...
  const isProteinActive = pathname?.startsWith(
    `/products/${CATEGORIES.PROTEIN}`
  );
  const isSportsNutritionActive = pathname?.startsWith(
    `/products/${CATEGORIES.SPORTS_NUTRITION}`
  );
  const isHealthActive = pathname?.startsWith(`/products/${CATEGORIES.HEALTH}`);
  const isFoodActive = pathname?.startsWith(`/products/${CATEGORIES.FOOD}`);
  const isVitaminActive = pathname?.startsWith(
    `/products/${CATEGORIES.VITAMIN}`
  );
    ...
    <>
      ...
      <div className="hidden md:flex 2xl:hidden flex-wrap items-center justify-center gap-2 md:gap-8 lg:gap-16 xl:gap-31 2xl:gap-16 py-2 px-3 bg-black text-white">
        ...
        <Link
          ...
          className="bg-transparent font-bold text-sm text-white uppercase focus:underline"
        >
          <span className="group inline-flex flex-row items-center gap-1">
            {CATEGORY_NAMES[CATEGORIES.PROTEIN]}
            <ChevronDown
              className={`w-4 h-4 transition-transform duration-200 group-active:rotate-180 ${
                isProteinActive ? "rotate-180" : ""
              } `}
            />
          </span>
        </Link>
        ...
        >
          <span className="group inline-flex flex-row items-center gap-1">
            ...
            <ChevronDown
              className={`w-4 h-4 transition-transform duration-200 group-active:rotate-180 ${
                isSportsNutritionActive ? "rotate-180" : ""
              } `}
            />
          </span>
        </Link>
      ...
      <div className="hidden 2xl:flex flex-row py-1 md:gap-2 lg:gap-10 xl:gap-10 2xl:gap-31 bg-black text-white justify-center pr-7 pl-1">
